...
class DatasetAnalyzer:
    ...
    def get_file_pairs(self) -> List[Tuple[str, str]]:
        """PCM과 텍스트 파일 쌍을 반환"""
        pairs = []
        for i in range(1, 2001):  # 2000개 파일
            pcm = f"KsponSpeech_{i:06d}.pcm"
            txt = f"KsponSpeech_{i:06d}.txt"
            if os.path.exists(os.path.join(self.data_dir, pcm)) and \
                    os.path.exists(os.path.join(self.data_dir, txt)):
