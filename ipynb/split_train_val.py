...
# 복사 함수 (진행률 포함)
def copy_pairs_with_progress(pairs, img_out, label_out, set_name):
    print(f"📁 {set_name} 데이터 복사 중...")
    
    # 초기 프로그레스바
    show_progress_bar(0, len(pairs), f"💾 {set_name} 복사")
    print()
    ...
    for idx, (img, lbl) in enumerate(pairs, 1):
        try:
            ...
            print(f"\n   ❌ 복사 실패: {os.path.basename(img)}, 오류: {e}")
            copy_errors += 1
        
        # 프로그레스바 업데이트
        show_progress_bar(idx, len(pairs), f"💾 {set_name} 복사")
    
    print(f"\n   ✅ {set_name} 완료 - 이미지: {copied_images}개, 라벨: {copied_labels}개")
    if copy_errors > 0:
        print(f"   ⚠️ 복사 오류: {copy_errors}개")
    print()

# 복사 실행
copy_pairs_with_progress(train_set, 
                        os.path.join(images_dir, 'train'), 
                        os.path.join(labels_dir, 'train'), 
                        "Train")

copy_pairs_with_progress(val_set, 
