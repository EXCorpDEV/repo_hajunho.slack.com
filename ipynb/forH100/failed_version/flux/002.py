...
class FluxInfiniteGenerator:
    ...
    def setup_model(self):
        ...
        try:
            ...
            # Flux 파이프라인 로드 - 더 안정적인 설정
            self.pipe = FluxPipeline.from_pretrained(
                self.model_name,
                torch_dtype=torch.bfloat16,
                use_safetensors=True,
                variant="bf16"  # bfloat16 variant 명시적 지정
            )
            ...
            print(f"파이프라인 구성 요소: {list(self.pipe.components.keys())}")
            ...
            print("3. 또는 다른 설정으로 재시도")
            
            # variant 없이 재시도
            try:
                print("variant 없이 재시도 중...")
                self.pipe = FluxPipeline.from_pretrained(
                    self.model_name,
                    torch_dtype=torch.bfloat16,
                    use_safetensors=True
