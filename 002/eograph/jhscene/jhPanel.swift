...
//
//  TODOLIST : DELETING duplicated property & method(property member of CALayer class, drawEllapse(), getX, getY, ...)
//

//             Making two classes to be separated from jhPanel is done
...
class jhPanel<T> : jhDraw, observer_p {
    ...
    let mMargin : CGFloat = jhGS.s.jhAMarginCommonV
    ...
    internal var mAllofCountOfDatas : Int {
        get {
            return jhDataCenter2.mDatas[jhPanelID]?.d.count ?? 0
        }
    }
...
//        }
        ...
        get {
            return (jhDraw.ARQ  - mMargin * 2) / CGFloat(jhDataCenter2.mCountOfdatas_view+1)
        }
//        set(distance) {
...
//        }
    ...
    var axisDistance: CGFloat {
        get {
            return (jhDraw.ARQ  - mMargin * 2) / CGFloat(jhDataCenter2.mCountOfaxes_view+1)
        }
    }
    ...
    override init(frame: CGRect) {
        if jhGS.s.logLevel.contains(.graphPanel) { print("jhPanel override init(\(frame.width), \(frame.height))")}
        super.init(frame: frame)
        
        self.layer.isGeometryFlipped = true
        
        mContext = UIGraphicsGetCurrentContext()
        if jhGS.s.logLevel.contains(.graphPanel) { print("jhPanel init color", mLineWidth)}
    }
    ...
    func drawPanel() {
        if jhGS.s.logLevel.contains(.graphPanel) { print("drawPanel()") }
    ...
    func drawLine(_ x1 : CGFloat, _ y1 : CGFloat, _ x2 : CGFloat, _ y2 : CGFloat) {
        if jhGS.s.logLevel.contains(.graph) { print("panel_drawLine_\(x1), \(y1), \(x2), \(y2)")}
    ...
    func initDatas() {
        ...
        for element in dataSource {
            ...
            jhDataCenter2.nonNetworkData.append(vNumber)
        }
        
        jhGS.s.testDataMaxValue = maxValue
        jhGS.s.testDataMinValue = minValue
        
        jhGS.s.testDataVerticalRatioToDraw_view = (jhDraw.ARQ - (2*mMargin)) / jhGS.s.testDataMaxValue
    }
