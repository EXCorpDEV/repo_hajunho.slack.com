...
class jhType2graphLayer<T> : jhCommonDataLayer<T>, jhLayer_p {
    //TODO: escaping hardcoding later on
    private let mMargin : CGFloat = 45
    private let hhhWidth : CGFloat = 86400 //24h
//    private let maxY : CGFloat = 25
    //margin 10000:300=25:0.75   0.75 *
    ...
    override func draw(in ctx: CGContext) {
        ...
        panelID = 1 //TODO: management should be in the same class which network class related with it
...
//                drawRectCustom(context: ctx, x1: fx, y1: fy, x2: fx2, y2: fy2)
                ...
                ctx.setFillColor(UIColor(red: 237, green: 176, blue: 231).cgColor)
                ctx.setStrokeColor(UIColor(red: 240, green: 151, blue: 232).cgColor)
                ctx.setLineWidth(1)
                
                let rectangle = CGRect(x: fx, y: 0.75 * yRatio, width: fx2-fx, height: fy)
