...
class jhDraw : UIView {
    ...
    /// A Regular Quadrilateral = vertual screen to draw and to calculate ratio
    internal static let ARQ : CGFloat = 10000.0
    
    static var defaultColor : CGColor = UIColor.blue.cgColor
    ...
    static func worldLine(context : CGContext?, _ x1 : Int, _ y1 : Int, _ x2 : Int, _ y2 : Int, _ lineWidth : CGFloat) {
        ...
        context?.setStrokeColor(self.defaultColor)
    ...
    static func worldLine(context : CGContext?, _ x1 : CGFloat, _ y1 : CGFloat, _ x2 : CGFloat, _ y2 : CGFloat, _ lineWidth : CGFloat, _ color : CGColor) {
        if jhGS.s.logLevel.contains(.graph) { print("draw_worldLine_\(x1), \(y1), \(x2), \(y2)")}
        context?.move(to: CGPoint(x: x1, y: y1))
        context?.addLine(to: CGPoint(x: x2, y: y2))
        context?.setStrokeColor(color)
        context?.setLineWidth(lineWidth)
    ...
    static func worldEllipse(context : CGContext?, _ x : CGFloat, _ y : CGFloat, _ width : CGFloat, _ height : CGFloat, _ lineWidth : CGFloat, _ color : CGColor) {
        if jhGS.s.logLevel.contains(.graph) { print("draw_worldEllipse_\(x), \(y), \(width), \(height)")}
