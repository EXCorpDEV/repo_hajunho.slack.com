...
class ChunkedVideoProcessor:
    ...
    def update_last_mask(self, mask, bbox):
        """마지막 마스크와 바운딩 박스 업데이트"""
        self.last_mask = mask
        self.last_bbox = bbox
    ...
    try:
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            with ChunkedVideoProcessor(args.video_path, chunk_size) as video_processor:
                ...
                current_frame_idx = 0
                last_mask = None
                last_bbox = prompts[0][0]  # 초기 바운딩 박스 저장
                ...
                while True:
                    ...
                    try:
                        with torch.inference_mode(), torch.autocast("cuda", dtype=torch.float16):
                            ...
                            # 매 청크마다 바운딩 박스 설정
                            _, _, masks = predictor.add_new_points_or_box(state, box=last_bbox, frame_idx=0, obj_id=0)

                            if masks is not None and len(masks) > 0:
                                last_mask = masks[0][0].cpu().numpy()
                            ...
                            for frame_idx, object_ids, masks in predictor.propagate_in_video(state):
                                ...
                                for obj_id, mask in zip(object_ids, masks):
                                    ...
                                    if len(non_zero_indices) > 0:
                                        ...
                                        last_bbox = (x_min, y_min, x_max - x_min, y_max - y_min)
